...
class IsAddHom (F: Type*) (α β: outParam Type*) [Add α] [Add β] [FunLike F α β]: Prop where
  protected resp_add: ∀f: F, ∀{x y}, f (x + y) = f x + f y
...
class IsMulHom (F: Type*) (α β: outParam Type*) [Mul α] [Mul β] [FunLike F α β]: Prop where
  protected resp_mul: ∀f: F, ∀{x y}, f (x * y) = f x * f y
...
class IsSMulHom (F R: Type*) (A B: outParam Type*) [FunLike F A B] [SMul R A] [SMul R B]: Prop where
  protected resp_smul (f: F): ∀{r: R} {x: A}, f (r • x) = r • f x

def resp_smul {F R α β: Type*} [FunLike F α β] [SMul R α] [SMul R β] [IsSMulHom F R α β] (f: F): ∀{r: R} {x: α}, f (r • x) = r • f x := IsSMulHom.resp_smul f

structure ZeroHom where
  toFun: α -> β
  protected resp_zero: toFun (0: α) = (0: β)
...
structure OneHom where
  toFun: α -> β
  protected resp_one: toFun (1: α) = (1: β)
...
structure AddHom where
  toFun: α -> β
  protected resp_add: ∀{x y: α}, toFun (x + y) = toFun x + toFun y
...
structure MulHom where
  toFun: α -> β
  protected resp_mul: ∀{x y: α}, toFun (x * y) = toFun x * toFun y
...
structure SMulHom (R A B: Type*) [SMul R A] [SMul R B] where
  toFun: A -> B
  protected resp_smul: ∀{r: R} {x: A}, toFun (r • x) = r • toFun x
...
instance : IsSMulHom (SMulEquiv R α β) R α β := ⟨SMulEquiv.resp_smul⟩
