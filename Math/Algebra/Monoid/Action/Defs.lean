...
class IsMulAction (R M: Type*) [MonoidOps R] [SMul R M] extends IsMonoid R: Prop where
  one_smul: ∀a: M, (1: R) • a = a
  mul_smul: ∀x y: R, ∀b: M, (x * y) • b = x • y • b

def one_smul [MonoidOps R] [SMul R M] [IsMulAction R M]: ∀a: M, (1: R) • a = a := IsMulAction.one_smul
def mul_smul [MonoidOps R] [SMul R M] [IsMulAction R M]: ∀x y: R, ∀b: M, (x * y) • b = x • y • b := IsMulAction.mul_smul

class IsDistribMulAction (R M: Type*) [MonoidOps R] [AddMonoidOps M] [SMul R M] [IsAddMonoid M] extends IsMulAction R M: Prop where
  smul_zero: ∀a: R, a • (0: M) = 0
  smul_add: ∀a: R, ∀x y: M, a • (x + y) = a • x + a • y

def smul_zero [MonoidOps R] [AddMonoidOps M] [SMul R M] [IsAddMonoid M] [IsDistribMulAction R M]: ∀a: R, a • (0: M) = 0 := IsDistribMulAction.smul_zero
def smul_add [MonoidOps R] [AddMonoidOps M] [SMul R M] [IsAddMonoid M] [IsDistribMulAction R M]: ∀a: R, ∀x y: M, a • (x + y) = a • x + a • y := IsDistribMulAction.smul_add
